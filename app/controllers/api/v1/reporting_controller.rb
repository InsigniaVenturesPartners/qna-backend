class Api::V1::ReportingController < Api::V1::BaseController
  ...
  def total_question_posted
    question = Question.group('DATE(created_at)').order('DATE(created_at) DESC').count
    return render :json => question
  end

  def total_answer_posted
    answer = Answer.where(question_id: Question.select('id')).group('DATE(created_at)').order('DATE(created_at) DESC').count
