...
a3 = Answer.create(body: "<p><span style=\"color: rgb(51, 51, 51);\">As many people have mentioned you can’t say “A programmer thinks x” or “A programmer does x”.</span></p><p><br></p><p><span style=\"color: rgb(51, 51, 51);\">I don’t have many years experience in programming, probably far less than many people who answered this question, but I know a rule.</span></p><p><br></p><p><span style=\"color: rgb(51, 51, 51);\">I would say that that rule is something like this: “If a programmer can come up with a efficient, easily maintained, easy to understand and easy solution to a problem that means that they know what they are doing.”</span></p><p><br></p><p><span style=\"color: rgb(51, 51, 51);\">Of course many more factors are important, but I think this is “on the top of the list”.</span></p>", author_id: user3.id, question_id: q2.id)

newa4 = Answer.create(body: "<p>Here are some of my favorites:</p><p><br></p><ul><li><strong>Miracle Sort</strong>&nbsp;-&nbsp;The alpha particles from the sun flip a few bits in the memory once a while, so this algorithm basically hopes that this flipping can cause the elements to be sorted. The way it works is:</li></ul><p><br></p><pre class=\"ql-syntax\" spellcheck=\"false\">1  Check if the array is sorted.\n2  While true:\n3    If the array is sorted, return the array.\n4    Else, wait for 10 seconds\n</pre><p>\t\t</p><p><br></p><ul><li><strong>Stack Sort</strong>&nbsp;- It searches for all the posts on StackOverflow with ‘sort a list’ in their title, extracts code snippets from them and then runs these code snippets until the list is sorted, which I think it verifies by actually traversing the list. It was posted on&nbsp;<a href=\"http://xkcd.com/\" target=\"_blank\" style=\"background-color: initial; color: rgb(43, 109, 173);\">xkcd</a>&nbsp;and an implementation can be found at&nbsp;<a href=\"https://gkoberger.github.io/stacksort/\" target=\"_blank\" style=\"background-color: initial; color: rgb(43, 109, 173);\">stacksort</a>&nbsp;.</li></ul><p><br></p><ul><li><strong>The Internet Sort</strong>&nbsp;- It’s just a bubble sort, but perform every comparison by searching the internet. For example, “Which is greater – 0.211 or 0.75?”.</li></ul><p><br></p>", author_id: user6.id, question_id: q3.id)
...
a12 = Answer.create(body: "<p>Instead of:</p><blockquote>Her: you're so quiet, what are you thinking about?</blockquote><blockquote>You: nothing!</blockquote><p>Try:</p><blockquote>Her: you're so quiet, what are you thinking about?</blockquote><blockquote>You: I was just spacing out for a second.</blockquote><p>Everyone can recognise that. You're still saying the same thing, you just come off more sincere.</p><blockquote>How do you explain to your [significant other] that sometimes you actually are thinking [of] nothing?</blockquote>", author_id: user7.id, question_id: q9.id)
...
t6 = Topic.create(name: "Life and Living", description: "The intervening period between birth and death.")

t7 = Topic.create(name: "Data Structures", description: "In Computer Science, a Data structure is a particular way of organizing data in a computer so that it can be used efficiently.
...
t8 = Topic.create(name: "Algorithms", description: "An algorithm is a precisely defined, effective and finite method for producing well-defined outputs from well-defined inputs.")

t9 = Topic.create(name: "Fine Dining", description: "Eating with class and style")

t10 = Topic.create(name: "Investing", description: "To commit (money or capital) in order to gain a financial return.")

t11 = Topic.create(name: "Harry Potter
(creative franchise)", description: "Harry Potter is a series of 7 fantasy novels by the author J K Rowling. The stories revolve around the titular character, Harry Potter, and his group of friends in a fictional wizarding world.

The series was also adapted into a 8 part movie series (the last book being split into 2 parts), all of which were major Hollywood blockbusters.

The 7 books have sold more than 400 million copies - with the first (Harry Potter and the Philosopher's stone) ranked the 4th highest selling book of all time).
The movie series is the highest grossing movie franchise of all time with estimated revenues of $7.7 bn. The Harry Potter brand is valued at $15 bn.")

t12 = Topic.create(name: "Machine Learning", description: "Machine learning is the study of computer algorithms that improve automatically through experience and has been central to AI research since the field's inception.")

t13 = Topic.create(name: "Time
(physics)", description: "Concepts, theories, descriptions, and accounts of \"time\". This is not a topic for cell phone programming, twitter, or other tangentially-related questions.")

t14 = Topic.create(name: "Philosophy", description: "The study of fundamental issues such as existence, and the nature of the world, often through systematic and critical discourse.")

t15 = Topic.create(name: "League of Legends
(video game)", description: "League of Legends is a computer game created by Riot Games. It qualifies into the MOBA (Multiplayer Online Battle Arena), or Defense of the Ancients: Allstars (2003 video game), genre; which came into being after the success of the \"Aeon of Strife\" custom map for Starcraft, and the extremely popular Warcraft III custom map \"Defense of the Ancients\".

The game has received awards such as Gamespy's Gamer's Choice Award for PC Game of the Year, as well as various GDC awards.

The game is free to download and play, and revenue is generated by  microtransactions that unlock character remodels (do not affect gameplay) or speed up acquisition of characters (all characters can be acquired without microtransactions).")

t16 = Topic.create(name: "Walt Disney World
", description: "The single most-visited tourist destination on the planet. The country's largest single-site employer. Dedicated to the Showman of the World, Walter Elias Disney.")

t17 = Topic.create(name: "Flirting", description: "Flirting or coquetry is a social and sometimes sexual activity involving verbal or written communication as well as body language by one person to another, suggesting an interest in a deeper relationship with the other person")

t18 = Topic.create(name: "Dating Advice", description: "Advice about dating.")

t19 = Topic.create(name: "The Onion", description: "The Onion is an American digital media company and news satire organization that publishes articles on international, national, and local news.")

t20 = Topic.create(name: "Stephen Hawking", description: "Stephen William Hawking was born on 8 January 1942 (300 years after the death of Galileo) in Oxford, England. At the age of eleven, Stephen went to St. Albans School and then on to University College, Oxford. Stephen wanted to study Mathematics, although his father would have preferred medicine. Mathematics was not available at University College, so he pursued Physics instead. After three years and not very much work, he was awarded a first  class honours degree in Natural Science. During his time at Cambridge Professor Hawking has written a large number of publications. The best way to read the most recent publications by Professor Hawking or his colleagues is by going to the SLAC SPIRES HEP site and searching for \"Find author Hawking\".")
...
q4.topics += [t1,t2,t9]
q5.topics += [t2,t6]
q6.topics += [t3,t6, t17, t18]
q7.topics += [t1,t5]
q8.topics += [t3,t6, t17, t18]
q9.topics += [t3, t17, t18]
q10.topics += [t2, t9]
q11.topics += [t1,t7]

#Create comments
Comment.destroy_all
c1 = Comment.build_from(newa4, user5.id, "Hahah, these sorts are great")
c2 = Comment.build_from(a1, user9.id, "so true")
...
c4 = Comment.build_from(a4, user6.id, "lol pretty slow...but not as slow as miracle sort!")

[c1,c2,c3,c4].each{|c| c.save!}
